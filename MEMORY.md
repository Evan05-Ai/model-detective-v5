...
> 当前版本: v3.0.4 后端（计费审计分型修正，神秘cc 0.05x 实测校准）+ Cosmic Galaxy v5.1 前端（资产 COSMIC_V300_20260906）
...
## 2026-09-07 v3.0.4 计费审计分型修正（重要，0.05x 倍率地面真值校准）

### 背景
v3.0.3 上线后用户实测成功（14 次请求全部跑完），并提供了关键地面真值：神秘cc 中转站面板显示**倍率 0.05x**、每请求实收 ~$0.013。对照审计截图发现计费审计存在三处误判+一处排序颠倒。

### 用户实测数字（校准基准，防丢失）
- 每请求上报 input ≈47,897（其中 cache_read=47,872），output ≈12
- 面板实收 $0.0132/请求 ≈ 官方价 $0.2395（opus-4-5 input $5/M）× 0.05x 倍率 ✓
- 14 次检测总实收 ~$0.18（用户真实成本极低）；同请求清洁直连仅 $0.000125
- **固定开销的构成**：cache_read 型（上游官方系统提示走缓存，官方计费仅 0.1x）——**不是** v3.0.3 披露里写的"缓存写入按 1.25 倍计费"，那次披露会把这个场景的用户成本高估 12.5 倍

### 修复（4 处）
1. **固定开销披露分型**：区分"缓存写入（1.25x）/缓存读取（0.1x）/非缓存注入（全价）"三段构成，给出"全价当量 tokens/请求"，文案从猜测改为结构化事实
2. **Cache 审计相干性判定**：缓存字段总规模 ≥1000 tokens（神秘cc 47.9k read、Kiro 类 45k read）→ 判"上游缓存透传"（OK 不扣分）；旧逻辑对这两个真实站都误报"可能虚报缓存计费 -30"。仅 input 小且 cache 字段微小/无来源时才保留 MAJOR 怀疑
3. **倍率提示与评分解耦**：>3x 时纯披露不扣分（旧代码 MINOR 提示之外 input 偏差又扣 25 分，同一事实双重记账）；消息明确标注"仅披露不扣分"
4. **小分母护栏**：估算基数 <20 tokens 时百分比无统计意义（偏差 251989% 这类数字），只披露不扣分；input 偏差扣分加绝对量门槛（>200 才扣 25、>100 才扣 10）。同一异常单一记账：固定开销已披露时 input 偏差不重复扣

### 场景得分验证（修正前 → 修正后）
| 场景 | 修正前 | 修正后 |
|------|--------|--------|
| 神秘cc（47.9k cache_read 透传） | 45（三重误扣） | **100**（披露+豁免） |
| Kiro 类（小 input + 45k read） | 60（误判虚报） | **90**（透传认定） |
| 真虚报（50 in + 30k read 无来源） | — | 100*（*规模门槛放行，见已知边界） |
| 诚实站 | — | 100 |
| 中等虚增（plain 500 in） | — | 75 |

### 已知边界（不过拟合声明）
- "虚报缓存"场景（小 input + 3 万 read）目前也判 100——规模门槛无法区分"大系统提示透传"与"伪造大额 read"。**这是有意取舍**：区分二者需要知道上游真实系统提示大小（不可得）；误伤诚实透传站（主流形态）比漏掉伪造站代价高，且固定开销披露已把数字摆给用户。若未来出现实锤伪造案例，再考虑"同站多次请求 read 完全相同才可疑"的启发式
- 倍率（0.05x 等）是站方定价参数，工具无法从 API 探知——报告只披露 token 事实，费用结论留给用户结合倍率计算，文档说明此边界

### 测试
- 115→116：新增 test_billing_integrity_relay_cache_overhead（神秘cc 截图真实数字固化回归，锁 100 分行为）；修正 test_billing_integrity_inflated（旧 Mock 的 15/25 tokens 低于估算不构成夸大，改真夸大场景 500 in）
- 全绿 116/116；烟测 /health v3.0.4-web 通过
