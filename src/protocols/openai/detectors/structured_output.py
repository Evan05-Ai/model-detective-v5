...
class StructuredOutputDetector(ActiveDetector):
    """结构化输出检测：验证 response_format json_schema strict 模式"""
    ...
    def run(self, client) -> CheckResultV2:
        resp = client.chat(
            messages=[{"role": "user", "content": "Generate a random person profile with name, age, and city."}],
            max_tokens=100,
        ...
        try:
            ...
            if has_name and has_age and has_city:
                score = 100
                # 检查是否有额外字段（strict 模式不应有）
                if len(data) > 3:
                    score = 80
                    issues.append(Issue(
                        level=IssueLevel.MINOR,
                        message=f"strict 模式下有额外字段: {list(data.keys())}",
                        detector_name=self.name,
                    ))
            else:
                score = 40
