...
class BasicRequestDetector(ActiveDetector):
    ...
    def run(self, client) -> CheckResultV2:
        ...
        if actual == claimed:
            ...
            # 规范化后相同（如 aiproxy-claude-opus-4.8 vs claude-opus-4-8）
            score = 75
            issues.append(Issue(
        ...
        if "choices" not in raw:
            score = min(score, 30)
            issues.append(Issue(level=IssueLevel.MAJOR, message="响应缺少 choices 字段", detector_name=self.name))
        ...
        if score >= 90:
            ...
            confidence = 0.8  # 低分但高置信度：明确检测到不匹配
